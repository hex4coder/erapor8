...
class Dapodik extends Command
{
                ...
                if($data->contains($satuan)){
                    ...
                    if($dapodik){
                        ...
                        if($this->argument('akses')){
                            $this->call('app:respon-artisan', ['status' => 'info', 'title' => 'Berhasil', 'respon' => 'Sinkronisasi '.$this->get_table($satuan).' berhasil']);
                        }
        ...
        try {
            ...
            if($sekolah->user){
                ...
                if($response){
                    ...
                    if($this->argument('akses')){
                        $this->call('app:respon-artisan', ['status' => 'error', 'title' => 'Gagal', 'respon' => 'Proses pengambilan data '.$this->get_table($satuan).' gagal. Server tidak merespon']);
                    }
            ...
            if($this->argument('akses')){
                $this->call('app:respon-artisan', ['status' => 'error', 'title' => 'Gagal', 'respon' => 'Proses pengambilan data '.$this->get_table($satuan).' gagal. Server tidak merespon. Status Server: '.$e->getMessage()]);
            }
            $this->proses_sync('', 'Proses pengambilan data '.$this->get_table($satuan).' gagal. Server tidak merespon', 0, 0, 0);
    ...
    private function proses_kd($data){
        $find = MataPelajaran::find($data->mata_pelajaran_id);
        if($find){
    ...
    private function simpan_peserta_didik_aktif($dapodik, $user, $semester){
        ...
        foreach($dapodik as $data){
            ...
            $a = PdKeluar::where('peserta_didik_id', $data->peserta_didik_id)->where('semester_id', $semester->semester_id)->delete();
            $this->proses_sync('Memperoses', 'peserta_didik_aktif', $i, count($dapodik), $user->sekolah_id);
            $bar->advance();
            $i++;
        }
        $bar->finish();
        /*AnggotaRombel::whereHas('rombongan_belajar', function($query) use ($semester){
    ...
    private function cari_wilayah($user, $semester, $kode_wilayah){
        $find = MstWilayah::find($kode_wilayah);
        if(!$find){
            $data = [
                'table' => 'ref.MstWilayah',
                'where' => 'kode_wilayah',
                'value' => $kode_wilayah,
                'limit' => '',
                'offset' => '',
    ...
    private function cari_mapel($user, $semester, $mata_pelajaran_id){
        $find = MataPelajaran::find($mata_pelajaran_id);
        if(!$find){
            $data = [
    ...
    private function insert_rombel($data, $user, $semester, $ekskul){
        ...
        /*$guru = Ptk::withTrashed()->find($data->ptk_id);
        if($guru){
            $jurusan = NULL;
            $jurusan_sp = NULL;
            if($data->jurusan_sp_id){
    ...
    private function simpan_rombongan_belajar($dapodik, $user, $semester){
        ...
        if($rombongan_belajar_id){
            RombonganBelajar::where('sekolah_id', $user->sekolah_id)->where('semester_id', $semester->semester_id)->whereNotIn('rombongan_belajar_id', $rombongan_belajar_id)->whereIn('jenis_rombel', [1, 16])->delete();
        }
    }
    private function simpan_ptk($dapodik, $user, $semester){
        ...
        //Ptk::where('sekolah_id', $user->sekolah_id)->where('is_dapodik', 1)->whereNotIn('guru_id_dapodik', $guru_id)->delete();
    ...
    private function simpan_ptk_keluar($dapodik, $user, $semester){
        if($dapodik){
            $guru = Ptk::withTrashed()->where('sekolah_id', $user->sekolah_id)->where('is_dapodik', 1)->whereNotIn('guru_id_dapodik', $dapodik)->get();
			foreach($guru as $data){
                PtkKeluar::updateOrCreate(
                    [
			...
			PtkKeluar::whereIn('guru_id', $dapodik)->where('sekolah_id', $user->sekolah_id)->delete();
            Ptk::onlyTrashed()->where('sekolah_id', $user->sekolah_id)->where('is_dapodik', 1)->whereIn('guru_id_dapodik', $dapodik)->restore();
        }
    }
    ...
    private function simpan_jurusan_sp($dapodik, $user, $semester){
        ...
        $bar->finish();
        JurusanSp::where('sekolah_id', $user->sekolah_id)->whereNotIn('jurusan_sp_id', $jurusan_sp_id)->delete();
    }
