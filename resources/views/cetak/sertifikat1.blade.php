...
@section('content')
    <div class="text-center" id="cover_utama">
        <img src="./images/logo/tutwuri.png" border="0" width="100" />
        <h2>SERTIFIKAT UJI KOMPETENSI</h2>
        <h3><i>CERTIFICATE OF COMPETENCY ASSESSMENT</i></h3>
        <br />
        Nomor :
        {{ date('Y', strtotime($rencana_ukk->tanggal_sertifikat)) . $sekolah->npsn . $siswa->peserta_didik->nisn . str_pad($count_penilaian_ukk, 5, 0, STR_PAD_LEFT) }}
        <br><br />
        ...
        <h4 style="padding-bottom:-10px;">
            <b>{{ $rencana_ukk->nilai_ukk ? keterangan_ukk($rencana_ukk->nilai_ukk->nilai) : '-' }}</b>
        </h4>
        <h4><strong><i>{{ $rencana_ukk->nilai_ukk ? keterangan_ukk($rencana_ukk->nilai_ukk->nilai, 'EN') : '-' }}</i></strong>
        </h4>
        <br>
