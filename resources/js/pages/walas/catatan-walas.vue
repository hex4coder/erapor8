...
const fetchData = async () => {
  ...
  try {
    ...
    getData.data_siswa.forEach((siswa) => {
      form.value.uraian_deskripsi[siswa.anggota_rombel.anggota_rombel_id] = siswa.anggota_rombel.catatan_walas?.uraian_deskripsi ?? null
    })
  } catch (error) {
    console.error(error);
  } finally {
...
<template>
  <VCard>
    <VCardItem class="pb-4">
      <VCardTitle>Catatan Wali Kelas</VCardTitle>
    </VCardItem>
